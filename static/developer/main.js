  ...
  fetch('/prices/prices')
    ...
    .then(pricesData => {
      ...
      activeCoins.forEach((coin, index) => {
        ...
        const buttonConfigs = [
            { 
                imageSrc: '/static/images/send.png', 
                onClick: () => {
                    const selectedWallet = wallets.find(wallet => 
                        wallet.ticker === coin.ticker && 
                        wallet.label === walletSelector.value
            ...
            { 
                imageSrc: '/static/images/receive.png', 
                onClick: () => {
                    const selectedWallet = wallets.find(wallet => 
                        wallet.ticker === coin.ticker && 
                        wallet.label === walletSelector.value
            ...
            { 
                imageSrc: '/static/images/mint.png', 
                onClick: () => {
                    const selectedWallet = wallets.find(wallet => 
                        wallet.ticker === coin.ticker && 
                        wallet.label === walletSelector.value
            ...
            { 
                imageSrc: '/static/images/NFTs.png', 
                onClick: () => {
                    const selectedWallet = wallets.find(wallet => 
                        wallet.ticker === coin.ticker && 
                        wallet.label === walletSelector.value
                    ...
                    if (selectedWallet) {
                        landingPage.innerHTML = '';
                        nftsUI(selectedWallet);
                    }
                }
            },
            { 
                imageSrc: '/static/images/tokens.png', 
                onClick: () => {
        ...
        // Center the buttons container and move it left by 10px
        buttons.style.display = 'flex';
        buttons.style.justifyContent = 'center';
        buttons.style.alignItems = 'center';
        buttons.style.margin = '0 auto';
        buttons.style.position = 'relative';
        buttons.style.left = '-10px'; // Move the container 10px to the left

        buttonConfigs.forEach(config => {
            const button = document.createElement('div');
            button.className = 'button';
            
            // Ensure the button has no background or border
            ...
            img.width = 60; // Size of the button
            img.height = 60;
